...
この文書はv1当時の実装記録です。以下の「未実装」「未完了」はv1時点の記録であり、
v2での搭載見送りを意味しません。v2の仕様は[ADR-0001](adr/0001-slack-bot-renewal.ja.md)、
使い方は[README](../../README.ja.md)を参照してください。

## v2での招待機能

- `scat channel create <name> --invite <users>`: 新規チャネルの作成時に招待します。
- `scat channel invite <channel> <user-or-group>...`: **既存チャネルに後から追加招待します。**

どちらも実装済みです。APIを注入するテストで検証しています。
実招待を伴うE2Eはpost/upload/exportの往復suiteとは別で、2026-09-22の実測には含めていません。
実装の有無と検証範囲は区別してください（[BUILD](BUILD.ja.md#実slack-e2e)）。
...
- ユーザーの指定は、ユーザーID（例: `U12345`）とメンション名（例: `@username`）の両方をサポートします。
  - v1当時は独立した `scat channel invite` がなく、作成時の招待だけでした。**v2では独立コマンドを実装しています。**
...
### ステップ6: ドキュメントの更新
- **`README.md`、`README.ja.md`**: 新しいコマンドの使用方法を追記する。（v1当時の記録では未完了。v2の使用方法は現在のREADMEに記載しています。）
