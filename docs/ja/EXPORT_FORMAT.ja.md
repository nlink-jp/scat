...
`scat export log` コマンドは、チャンネルのメッセージ履歴を構造化されたJSON形式で出力します。`messages` 配列の各エントリは1件のメッセージを表し、以下のフィールドを含みます。
...
- `files` (array of objects, optional): メッセージに添付ファイルが含まれる場合の、ファイルオブジェクトの配列。
  - `id` (string): ファイルのID。
...
- `thread_timestamp_unix` (string, optional): メッセージが返信である場合、スレッドの親メッセージのUnixタイムスタンプ。
- `is_reply` (bool): メッセージがスレッド内の返信である場合は `true`、そうでない場合は `false`。
