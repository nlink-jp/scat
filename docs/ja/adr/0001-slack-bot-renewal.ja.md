...
（配布するマニフェストについては本記録末尾の2026-09-23の追補を参照）
...
### マニフェスト権限についての追補 — 2026-09-23

配布する `slack-app-manifest.json` は、§6のscopeの和集合を要求する。操作ごとに分けていない。
マニフェストは、その導入先がどの操作を使うか決まる前に一度だけ取り込むもので、Slackは全体として適用する。
操作ごとに分けると、運用者がまだ判断できない選択を迫り、機能を使い足すたびに再インストールが要る。
和集合はこのCLIが使い得る範囲そのもので、それ以上は含まない。`manifest_test.go` が双方向に照合する。

コードが呼んでよい範囲の規則は§6のままとする。権限を絞りたい運用者は、取り込む前に写しからscopeを消す。
消した操作は、method・code・必要scopeを添えて失敗する。これは§6が元から要求している失敗のしかたである。
